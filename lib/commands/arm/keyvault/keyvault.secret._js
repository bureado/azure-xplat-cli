...
exports.init = function(cli) {
  ...
  secret.command('set [vault-name] [secret-name] [secret-value]')
    ...
    .option('--secret-source <secret-source>', $('source file for the secret value'))
  ...
  function parseSecretPropertiesArguments(vaultName, secretName, secretVersion, value, options, requireValue) {
    ...
    if(options.secretSource) {     
      log.verbose('Reading secret from source file: ' + options.secretSource);
      options.value = fs.readFileSync(options.secretSource).toString();
    }
